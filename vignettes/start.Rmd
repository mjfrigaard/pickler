...
Adopting test [fixtures, helpers, and setup](https://www.tidyverse.org/blog/2020/04/self-cleaning-test-fixtures/) files will address the first item in the list above, but `bddR` is mostly focused on the second item: making test obvious and clear. 
...
`bddR` address the advice above by providing a set of [Gherkin-style syntax](https://cucumber.io/docs/gherkin/reference/) functions that can be palced in the `description` argument of `decribe()` or `it()`. The following sections walk through an example with the `split_cols()` function: 
...
The three primary functions in `bddR` are `feature()`, `scenario()`, and `background()`.
...
The point of a feature is to accurately capture what we are trying
to achieve with a function (or set of functions, application, package, etc.) 
...
```gherkin
  Scenario: Split column with a default pattern
    Given a dataframe with a specified column "<column_name>"
    When I split the "<column_name>" column using the default pattern
    Then the "<column_name>" column should be split into multiple columns
```

Below is the same scenario in the `scenario()` function:
...
When rendered, scenarios look like so:
...
In Gherkin you'll occasionally see `And` keywords following the `Given` statement, but in `bddR` these are intended to be placed in the `background()` function:
...
These `And` statements can be placed in the `background()` to removes repetition:
...
The nice thing about storing context in a `background()` means that any updates will only have to change in a single location.
...
BDD tests should communicates the context and intended behaviors of a function, and provides a series of worked examples for how the code should be used. Writing BDD tests takes longer, but makes it easier for stakeholders and other developers to understand how the code in your package should be used (and tested).
...
It clearer to use nested `describe()` and `it()` calls, because they can hold combinations of features, scenarios, or backgrounds. Below is an example feature(), `background()`, and `scenario()` for `split_cols()`
...
Ideally, `bddR`'s functions are developed independently and then wrapped in a `bundle()` before passing to `testthat`'s BDD functions: 
...
This makes it easier to combine features and backgrounds into a single call:
...
This allows us to pass a feature and background `bundle()` to `describe()` and test separate `scenario()`s:
...
It's also possible to provide a table in a scenario or feature. Let's assume we have a small example of data that illustrates an example behavior for our `split_cols()` function (stored in `input_data`):
...
Now our `background`, `feature`, `scenario` (and tables) are in the Gherkin-style format that can be placed in R Markdown and nicely rendered for stakeholders or non-technical audiences: 
...
And when it's placed in the test file, it passes!
