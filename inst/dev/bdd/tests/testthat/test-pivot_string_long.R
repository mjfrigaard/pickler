describe("Feature: Pivot Single String into Long Format
  ...
  So that I can analyze each unique item in the string separately.", code = {
    it("Scenario 1: Pivot a single string using a default separator
               ...
               And the 'string' column should contain the original string for each unique item", code = {
    input_string <- "a-b-c"
